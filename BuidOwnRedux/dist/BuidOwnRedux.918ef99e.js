...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"0MJlQ"}],"0MJlQ":[function(require,module,exports,__globalThis) {
...
const wishListReducer = (state = initialState, action)=>{
    switch(action.type){
        ...
        case REMOVEWISHLISTITEM:
            {
                const { ProductID } = action.payload;
                const found = state.wishList.find((cartitem)=>cartitem.ProductID === ProductID);
                if (found) {
                    const deleted = state.wishList.filter((cartitem)=>cartitem.ProductID !== ProductID);
                    return {
                        ...state,
                        wishList: deleted
                    };
